...
class Contacts(View):
    ...
    def post(self, request, **kwargs):
        ...
        if request.user.is_authenticated:
            has_connected = Contact.objects.filter(car_id=car_id, user_id=user_id)
            if has_connected:
                messages.error(request, "You have already sent us an inquiry about this car...please wait!!!")
        ...
        contact = Contact(car_id=car_id, car_title=car_title, user_id=user_id,
                          ...
                          city=city, state=state, email=email, phone=phone, message=message).save()
